 ...
 * Class HasMany.
 */
class HasManyNestable extends Field
{
    /**
     * Relation name.
     *
    ...
    protected $views = [
        ...
        'nestable'  => 'admin::form.hasmanynestable',
    ];
...
$('#has-many-{$this->column}').on('click', '.add', function () {
    ...
    $('.has-many-{$this->column}-forms').find('div.col-sm-8 > input.la_checkbox').each(function (){
        var name =  $(this).attr('name');       
        $('input[name="'+ name +'"].la_checkbox').bootstrapSwitch({
            size:'small',
            onText: 'ON',
            offText: 'OFF',
            onColor: 'primary',
            ...
            onSwitchChange: function(event, state) {
                 $('input[name="'+ name +'"]').val(state ? 'on' : 'off');
            }
        });
    });
...
EOT;
    ...
    /**
     * Setup nested template script.
     *
     * @param string $templateScript
     *
     * @return void
     */
    protected function setupScriptForNestableView($templateScript)
    {
        $removeClass = NestedForm::REMOVE_FLAG_CLASS;
        $defaultKey = NestedForm::DEFAULT_KEY_NAME;
        ...
        $('#has-many-{$this->column}').nestable();
        //$('.dd').nestable();
        $('#has-many-{$this->column}').on('change', function() {
            {$this->column}_change_order();
        });
...
$('#has-many-{$this->column}').on('click', '.add', function () {
    ...
    var last = {$this->column}_change_order();
    var container = $('input.order[value='+last+']').closest('.dd-content');
    var text = container.find('.main-selector').find('option:selected').html();
    container.siblings('.dd-handle').html(text);
...
$('#has-many-{$this->column}').on('click', '.dd-content-view', function () {
    $(this).siblings('.dd-content').toggleClass('in');
});

$('#has-many-{$this->column}').on('change', '.main-selector', function () {
   var text = $(this).find('option:selected').html();
    $(this).closest('.dd-content').siblings('.dd-handle').html(text);
});

$('#has-many-{$this->column}').find('.dd-handle').each(function(){
    var text = $(this).find('select option:selected').html();
    $(this).html(text);   
    
});

//$('#has-many-{$this->column}').find('.dd-content').each(function(){
//    var id = $(this).data('id');   
//    $(this).find('.form-group').find('[name ^= {$this->column}]').each(function(){
// 
//        var name = $(this).attr('name');
//           console.log(name);
//        name.replace(/['+id+']/g,'[new_'+id+']');
//         console.log(name);
//        $(this).attr('name',name);
//    });
//     
...
function {$this->column}_change_order(el){
    var order = 0;
    $('#has-many-{$this->column}').find('input.order').each(function(){    
        $(this).val(++order);       
    });
